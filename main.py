    ...
    try:
        ...
        if request.json['trade'] == 'demotrade' and request.json['desire'] == 'create':
            ...
            try:
                ...
                try:
                    bullets_len = len(
                        mbr_col.document(_email).collection('trading').document('DemoTrade').get().to_dict())
                ...
                for i in range(20):
                    ...
                    add_data.update({
                        str(i + bullets_len + 1): {'amount': 5, 'status': 'Bought', 'comment': '', 'sell': '',
                                                   'buy': offer},
                    })
        ...
        if mbr_data['ident'] > 5:
            ...
            if request.json['trade'] == 'ClubFeedXRP_hand':
                ...
                try:
                    uid_ = auth.get_user_by_email(_email).uid
                    if request.json['cashflow'] is not None:
                        trd_doc.update({
                            uid_: {'email': _email, 'ident': request.json['ident'],
                                   'cashflow': request.json['cashflow']}
                        })

                    try:
                        bullets_len = len(mbr_col.document(_email).collection('trading').document(
                            'ClubFeedXRP_hand_bullets').get().to_dict())
                    except Exception as e:
                        print(e)
                        bullets_len = 0

                    add_data = {}
                    if request.json['price'] is None:
                        buy_ = ''
                    else:
                    ...
                    for i in range(int(request.json['bullet'])):
                        add_data.update({
                            str(i + bullets_len + 1): {'amount': request.json['amount'], 'status': 'Bought',
                                                       'comment': '', 'sell': '', 'buy': buy_},
                        })
            ...
            # ClubFeedXRP_bot
                ...
                try:
                    ...
                    try:
                        bullets_len = len(mbr_col.document(_email).collection('trading').document(
                            'ClubFeedXRP_bot_bullets').get().to_dict())
                    except Exception as e:
                        print(e)
                        bullets_len = 0
                    ...
                    for i in range(int(request.json['bullet'])):
                        add_data.update({
                            str(i + bullets_len + 1): {'amount': request.json['amount'], 'status': 'Bought', 'sell': '',
                                                       'buy': request.json['price'], 'sell_pending': 0,
                                                       'buy_pending': 0},
                        })
...
def clubfeedxrp_bot_table():
    ...
    if request.json['method'] == 'cancel':
        ...
        return jsonify({'message': 'success', 'sell_pending': onc_blt_data['sell_pending'],
                        'buy_pending': onc_blt_data['buy_pending']})
...
def index(dinamic):
    if '_session' not in request.cookies:
        ...
        try:
            ...
            if dinamic == '':
                return render_template('index.html', title=_title, dinamic=dinamic, mbr_email=mbr_email,
                                       mbr_data=mbr_data)
                ...
                try:
                    ...
                    trading = mbr_col.document(mbr_email).collection('trading').document(
                        dinamic + '_bullets').get().to_dict()
                    ...
                    print(e)
                    trading = {}
                return render_template('index.html', title=_title, dinamic=dinamic, trading=trading,
                                       mbr_email=mbr_email, mbr_data=mbr_data, trd_data=trd_data)
            ...
            try:
                trd_doc.update({
                    auth.get_user_by_email(email).uid: {'email': email, 'ident': copx.get().to_dict()['ident'],
                                                        'cashflow': copy.get().to_dict()['cashflow']}
                })
            except:
                trd_doc.update({
                    auth.get_user_by_email(email).uid: {'email': email, 'ident': copx.get().to_dict()['ident']}
