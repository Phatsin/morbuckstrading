...
from flask import Flask, render_template, redirect, request, url_for, session, jsonify, abort, make_response, g
    ...
    if mbr_data['ident'] > 5:
      ...
      if request.json['trade'] == 'ClubFeedXRP_hand':
        ...
        try:
          ...
          try:
            bullets_len = len(mbr_col.document(_email).collection('trading').document('ClubFeedXRP_hand_bullets').get().to_dict())
          except:
            bullets_len = 0
      ...
      #ClubFeedXRP_bot
        ...
        try:
          ...
          try:
            bullets_len = len(mbr_col.document(_email).collection('trading').document('ClubFeedXRP_bot_bullets').get().to_dict())
          except:
            bullets_len = 0
